...
const Map = () => {
  ...
  // Effect to trigger continuous bouncing animation
  useEffect(() => {
    const bounceMarker = () => {
      setMarkerAnimation(window.google.maps.Animation.BOUNCE);
      setTimeout(() => {
        setMarkerAnimation(null); // Stop animation for a moment to reset
      }, 700); // Reset after one bounce cycle
    };

    const bounceInterval = setInterval(bounceMarker, 1500); // Repeat every 1.5 seconds
